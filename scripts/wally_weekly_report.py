#!/usr/bin/env python3
"""
Wally Health - Weekly Google Ads Performance Report
...
def generate_action_items(l7, p7, campaigns, keywords):
    ...
    # WoW spend change
    if l7 and p7:
        spend_change = (l7["cost"] - p7["cost"]) / p7["cost"] * 100 if p7["cost"] > 0 else 0
        ...
        if conv_change < -20:
            actions.append(f"🚨 Conversions down {conv_change:.0f}% WoW — check for campaign learning phase disruptions or tracking issues.")

    # Campaign-level IS flags
    for c in campaigns:
        if c["is_lost_rank"] > 0.7 and c["conversions"] > 0:
            actions.append(f"⚠️ {c['name']}: {c['is_lost_rank']:.0%} IS Lost to Rank — still in learning phase, monitor tCPA / conversion volume.")
        if c["is_lost_budget"] > 0.2:
            actions.append(f"⚠️ {c['name']}: {c['is_lost_budget']:.0%} IS Lost to Budget — consider increasing daily budget (${c['budget']}/day).")
        if c["cpa"] > 400 and c["conversions"] > 0:
            actions.append(f"⚠️ {c['name']}: CPA ${c['cpa']} — above threshold, review bidding strategy and conversion signal.")

    # Keyword-level flags
    for kw in keywords:
        if kw["qs"] in (3, 4) and kw["cost"] > 100:
            actions.append(f"⚠️ Keyword \"{kw['text']}\" (QS {kw['qs']}) spent ${kw['cost']} — low QS causing bid penalty, review ad copy alignment.")
        if kw["conversions"] == 0 and kw["cost"] > 200:
            actions.append(f"⚠️ Keyword \"{kw['text']}\" spent ${kw['cost']} with 0 conversions — consider pausing or restructuring ad group.")

    if not actions:
        actions.append("✅ No critical issues detected this week. Continue monitoring learning phase recovery.")

    return actions


def build_html(l7, p7, l30, campaigns_l7, campaigns_l30, keywords_l7, actions, run_date):
    def row_color(val, good_direction="down"):
        """Return green if improved, red if worsened."""
        if val is None or val == "N/A":
            return ""
        try:
            v = float(val.replace("%", "").replace("+", ""))
            if good_direction == "down":
                color = "#d4edda" if v < 0 else "#f8d7da" if v > 10 else ""
            else:
                color = "#d4edda" if v > 0 else "#f8d7da" if v < -10 else ""
        except Exception:
            return ""
...
<body>
<h1>📊 Wally Health — Weekly Ads Report</h1>
<p style="color:#6c757d;">Week of {run_date} &nbsp;|&nbsp; Wally Health (Account: 4459200652)</p>
"""
    ...
    if l7 and p7:
        ...
        for label, val, wow, good_dir in metrics:
            try:
                v = float(wow.replace("%", "").replace("+", ""))
                if good_dir == "up":
                    wow_class = "good" if v > 0 else "bad" if v < -10 else "neutral"
                else:
                    wow_class = "good" if v < 0 else "bad" if v > 10 else "neutral"
            except Exception:
                wow_class = "neutral"
        ...
        <div class="metric-box">
          ...
          <div class="metric-wow <{wow_class}">{wow} WoW</div>
        </div>"""
        html += "</div>"

    # --- Action items ---
    html += "<h2>🎯 Suggested Action Items</h2>"
    for action in actions:
        html += f'<div class="action-item">{action}</div>'

    # --- Campaign performance L7 ---
    html += "<h2>Campaign Performance — Last 7 Days</h2>"
    html += """<table>
<tr><th>Campaign</th><th>Type</th><th>Cost</th><th>Conv</th><th>CPA</th><th>IS</th><th>IS Lost Rank</th><th>IS Lost Budget</th></tr>"""
    for c in campaigns_l7:
        is_str = fmt_pct(c["is"]) if c["is"] else "N/A"
        ilr_str = fmt_pct(c["is_lost_rank"]) if c["is_lost_rank"] else "N/A"
        ilb_str = fmt_pct(c["is_lost_budget"]) if c["is_lost_budget"] else "N/A"
...
<td>{is_str}</td><td{ilr_class}>{ilr_str}</td><td{ilb_class}>{ilb_str}</td>
</tr>"""
    html += "</table>"

    # --- Campaign performance L30 ---
    html += "<h2>Campaign Performance — Last 30 Days</h2>"
    html += """<table>
<tr><th>Campaign</th><th>Cost</th><th>Conv</th><th>CPA</th><th>IS</th><th>IS Lost Rank</th><th>Budget/Day</th></tr>"""
...
<td>{c['name']}</td>
<td>${c['cost']:,}</td><td>{c['conversions']}</td><td>${c['cpa']}</td>
<td>{is_str}</td><td{ilr_class}>{ilr_str}</td><td>${c['budget']}/day</td>
</tr>"""
    html += "</table>"
    ...
    html += """<table>
<tr><th>Keyword</th><th>Match</th><th>QS</th><th>Cost</th><th>Conv</th><th>CPA</th><th>IS</th><th>Lost Rank</th></tr>"""
    for kw in keywords_l7:
        qs = kw["qs"] if kw["qs"] else "—"
        qs_class = ' class="bad"' if kw["qs"] and kw["qs"] <= 4 else ''
        is_str = fmt_pct(kw["is"]) if kw["is"] else "N/A"
        ilr_str = fmt_pct(kw["is_lost_rank"]) if kw["is_lost_rank"] else "N/A"
        html += f"""<tr>
<td>"{kw['text']}"</td><td>{kw['match']}</td><td{qs_class}>{qs}</td>
<td>${kw['cost']}</td><td>{kw['conversions']}</td><td>${kw['cpa']}</td>
<td>{is_str}</td><td>{ilr_str}</td>
</tr>"""
    html += "</table>"
...
def send_email(html, run_date):
    ...
    msg["Subject"] = f"Wally Health — Weekly Ads Report ({run_date})"
